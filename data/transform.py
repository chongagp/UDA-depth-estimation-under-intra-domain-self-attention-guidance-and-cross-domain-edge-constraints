...
class RandomPairedCrop(object):
    ...
    def __call__(self, img):
        ...
        if edge is not None:
            edge = F.crop(edge, i, j, th, tw)
        return img1, img2, depth, edge
...
class RandomImgAugment(object):
    ...
    def __call__(self, inputs):
        ...
        if edge is not None:
            edge = scale_transform(edge)

        if fb is not None:
            scale = float(self.size[1]) / float(w0)
            fb = fb * scale
        if phase == 'test':
            return img1, img2, depth, fb, edge
        ...
        if not self.size == 0:
            ...
            if depth is not None:
                ...
                arr_depth /= 65535.0  # cm->m, /10,uiyihua
        ...
        if self.rotation and not (img2 is not None and depth is not None):
            if random.random() < 0.5:
                ...
                if edge is not None:
                    edge = F.rotate(edge, degree, Image.BILINEAR)
                if img2 is not None:
        ...
        if self.augment:
            if random.random() < 0.5:
                ...
                if edge is not None:
                    edge = F.adjust_brightness(edge, brightness)
                    edge = F.adjust_contrast(edge, contrast)
                    edge = F.adjust_saturation(edge, saturation)
