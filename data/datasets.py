...
class VKittiDataset(data.Dataset):
    def __init__(self, root='./datasets', data_file='src_train.list',
                 ...
                 joint_transform=None,img_transform_edge=None):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        if self.joint_transform is not None:
            if self.phase == 'train':    
                img, _, depth, _, _ = self.joint_transform((img, None, depth, self.phase, None,None))
            else:
                img, _, depth, _, _ = self.joint_transform((img, None, depth, 'test', None, None))
...
class KittiDataset(data.Dataset):
    def __init__(self, root='./datasets', data_file='tgt_train.list', phase='train',
                 img_transform=None, joint_transform=None, depth_transform=None,img_transform_edge=None):
        ...
        self.joint_transform = joint_transform
        self.depth_transform = depth_transform
        self.img_transform_edge = img_transform_edge
        with open(osp.join(self.root, self.data_file), 'r') as f:
            data_list = f.read().split('\n')
            for data in data_list:
                if len(data) == 0:
    ...
    def read_data(self, datafiles):
        ...
        assert osp.exists(osp.join(self.root, datafiles['edge'])), "edge does not exist"
        edge = Image.open(osp.join(self.root, datafiles['edge']))
        return l_rgb, r_rgb, fb, depth, edge
    ...
    def __getitem__(self, index):
        ...
        if index > len(self)-1:
            index = index % len(self)
        datafiles = self.files[index]
        l_img, r_img, fb, depth, edge = self.read_data(datafiles)
        # l_img, r_img, fb, depth = self.read_data(datafiles)
            
        if self.joint_transform is not None:
            if self.phase == 'train':
                l_img, r_img, _, fb, edge = self.joint_transform((l_img, r_img, None, 'train', fb, edge))
            else:
        ...
        if self.img_transform_edge is not None:
            edge = self.img_transform_edge(edge)
...
def get_dataset(root, data_file='train.list', dataset='kitti', phase='train',
                ...
                joint_transform=None, test_dataset='kitti',img_transform_edge=None):
    ...
    return name2obj[dataset.upper()](root=root, data_file=data_file, phase=phase,
                                     ...
                                     joint_transform=joint_transform,img_transform_edge=img_transform_edge)
