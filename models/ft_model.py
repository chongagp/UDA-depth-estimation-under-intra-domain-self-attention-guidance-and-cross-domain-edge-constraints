...
class FTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_L_Edge', type=float, default=1.0, help='weight for e loss')
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            self.visual_names = ['src_img', 'src_real_depth', 'src_gen_depth', 'tgt_left_img', 'tgt_right_img', 'fake_src_left', 'tgt_gen_depth', 'warp_tgt_img','out_edge']
        else:
        ...
        self.netG_Depth_T = networks.init_net(networks.UNetEB5Model(norm='batch'), init_type='normal', gpu_ids=opt.gpu_ids)
        # self.Depth_edge = networks.init_net(networks._UNetSAGenerator(norm='batch'), init_type='normal', gpu_ids=opt.gpu_ids)
        self.netDepth_edge = networks.init_net(networks.UNet(norm='batch'), init_type='normal', gpu_ids=opt.gpu_ids)

        # self.netG_Depth_T = networks.init_net(networks._UNetSAGenerator(norm='batch'), init_type='normal', gpu_ids=opt.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_G_task = torch.optim.Adam(itertools.chain(self.netG_Depth_T.parameters(),self.netDepth_edge.parameters()),
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            self.s_out_edge = self.out_edge.narrow(0, self.num, self.num)
