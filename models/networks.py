...
class UNetEB5Model(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, ngf=64, layers=4, norm='batch', activation='PReLU', drop_rate=0,
                 add_noise=False, gpu_ids=[],
                 weight=0.1):
        super(UNetEB5Model, self).__init__()
        self.sa1 = ScalarSelfAttention(176)
        ...
        # self.encoder_pre_trained = EfficientNet.from_pretrained('efficientnet-b5')
        ...
        self.deconv4 = _DecoderUpBlock(129, 128, 40, norm_layer, nonlinearity, use_bias)
        self.deconv3 = _DecoderUpBlock(81, 80, 24, norm_layer, nonlinearity, use_bias)
        self.deconv2 = _DecoderUpBlock(49, 48, 12, norm_layer, nonlinearity, use_bias)

        self.output5 = _OutputBlock(352, output_nc, 3, use_bias)
        self.output4 = _OutputBlock(129, output_nc, 3, use_bias)
        self.output3 = _OutputBlock(81, output_nc, 3, use_bias)
        self.output2 = _OutputBlock(49, output_nc, 3, use_bias)
        self.output1 = _OutputBlock(13, output_nc, 7, use_bias)
    ...
    def forward(self,input):
        ...
        conv1 = features['reduction_1']#24
        conv2 = features['reduction_2']#40
        conv3 = features['reduction_3']#64
        conv4 = features['reduction_4']#176
        conv5 = features['reduction_6']#2048
        scale = 1.0
        result = []
        center_out = self.center.forward(conv5)
        center_out, amap1 = self.sa1(center_out)
        deconv5 = self.deconv5.forward(torch.cat([center_out, conv4 * self.weight], 1))
        output5 = scale * self.output5.forward(torch.cat([center_out, conv4 * self.weight], 1))
        deconv4 = self.deconv4.forward(torch.cat([deconv5, conv3 * self.weight * 0.5, self.upsample(output5)], 1))
        output4 = scale * self.output4.forward(torch.cat([deconv5, conv3 * self.weight * 0.5, self.upsample(output5)], 1))
...
class UNet(nn.Module):
    def __init__(self, input_nc=1, output_nc=1, ngf=16, layers=4, norm='batch', drop_rate=0, add_noise=False, weight=0.1):
        ...
        self.conv3 = _EncoderBlock(ngf*2, ngf*4, ngf*4, norm_layer, nonlinearity, use_bias)
        self.conv4 = _EncoderBlock(ngf*4, ngf*8, ngf*8, norm_layer, nonlinearity, use_bias)
        self.conv5 = _EncoderBlock(ngf * 8, ngf * 8, ngf *8, norm_layer, nonlinearity, use_bias)
        ...
        self.deconv4 = _DecoderUpBlock(ngf * (8 + 8), ngf * 16, ngf * 4, norm_layer, nonlinearity, use_bias)
        self.deconv4_1 = _DecoderUpBlock(ngf * 8 , ngf * 8, ngf * 8, norm_layer, nonlinearity, use_bias)
        self.deconv3 = _DecoderUpBlock(ngf*(4+4), ngf*8, ngf*2, norm_layer, nonlinearity, use_bias)
        self.deconv2 = _DecoderUpBlock(ngf*(2+2), ngf*4, ngf*1, norm_layer, nonlinearity, use_bias)
        self.deconv1 = _DecoderUpBlock(ngf*(1+1), ngf*2, int(ngf/2), norm_layer, nonlinearity, use_bias)

        self.output4 = _OutputBlock(ngf*(4+4), output_nc, 3, use_bias)
        self.output3 = _OutputBlock(ngf*(2+2)+output_nc, output_nc, 3, use_bias)
        self.output2 = _OutputBlock(ngf*(1+1)+output_nc, output_nc, 3, use_bias)
        self.output1 = _OutputBlock(int(ngf/2), output_nc, 7, use_bias)
    ...
    def forward(self, input):
        ...
        center_in = self.pool(self.conv5.forward(conv4))

        scale = 1.0
        result = []
        deconv4_1 = self.deconv4_1.forward(center_in)
        deconv4 = self.deconv4.forward(torch.cat([deconv4_1, conv4 ], 1))
        deconv3 = self.deconv3.forward(torch.cat([deconv4, conv3 ], 1))
        deconv2 = self.deconv2.forward(torch.cat([deconv3, conv2 ], 1))
        deconv1 = self.deconv1.forward(torch.cat([deconv2, conv1 ], 1))
        output1 = scale * self.output1.forward(deconv1)
