...
class Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_L_Edge', type=float, default=150, help='weight for edge loss')
            parser.add_argument('--lambda_S_Depth', type=float, default=0.01,
                                help='weight for smooth loss')
            
            parser.add_argument('--lambda_R_Img', type=float, default=50,help='weight for image reconstruction')
            # cyclegan
            ...
            parser.add_argument('--t_depth_premodel', type=str, default=" ",
                                help='pretrained depth estimation model')
            parser.add_argument('--edge_premodel', type=str, default=" ",
                                help='pretrained edge model')
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            visual_names_tgt = ['tgt_left_img', 'fake_src_left', 'tgt_gen_depth', 'warp_tgt_img_s', 'warp_tgt_img_t', 'tgt_gen_depth_s', 'tgt_gen_depth_t', 'tgt_right_img','s_out_edge','t_out_edge']
        ...
        if self.isTrain:
            self.model_names = ['G_Depth_S', 'G_Depth_T','Depth_edge']
        ...
        self.netG_Depth_S = networks.init_net(networks.UNetEB5Model(norm='batch'), init_type='kaiming', gpu_ids=opt.gpu_ids)
        self.netG_Depth_T = networks.init_net(networks.UNetEB5Model(norm='batch'), init_type='kaiming', gpu_ids=opt.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.init_with_pretrained_model('Depth_edge', self.opt.edge_premodel)
        ...
        if self.isTrain:
            ...
            self.optimizer_G_task = torch.optim.Adam(itertools.chain(self.netG_Depth_S.parameters(),
                                                                    self.netG_Depth_T.parameters(),self.netDepth_edge.parameters()),
            ...
            if opt.freeze_bn:
                ...
                self.netDepth_edge.apply(networks.freeze_bn)
            if opt.freeze_in:
    ...
    def backward_G(self):
        ...
        self.out_edge = self.netDepth_edge(torch.cat((self.tgt_gen_depth_t, self.tgt_gen_depth_s), 0))
        self.s_out_edge = self.out_edge.narrow(0, 0, self.num)
        ...
        self.loss_l_Edge_Imgs += self.criterionDepthReg(self.s_out_edge, edge_imgs) * lambda_L_Edge

        self.loss_r_Edge_Imgs = 0.0
        self.loss_r_Edge_Imgs += self.criterionDepthReg(self.t_out_edge, edge_imgs) * lambda_L_Edge

        self.loss_G = self.loss_G_Tgt + self.loss_cycle_Tgt + self.loss_idt_Src + self.loss_R_Img_Tgt_T + self.loss_R_Img_Tgt_S + self.loss_S_Depth_Tgt_T + self.loss_S_Depth_Tgt_S + self.loss_C_Depth_Tgt+ self.loss_l_Edge_Imgs+ self.loss_r_Edge_Imgs
